...
func _init(trigger_key: Key, ms_detect_input: int) -> void:
...
func input(event: InputEvent) -> void:
	if event is InputEventKey:
		...
		if key_event.keycode == _trigger_key and key_event.is_pressed():
			if _trigger_key_was_pressed and (Time.get_ticks_msec() - _last_time_trigger_key_pressed) < _ms_detect_input:
				_trigger_key_was_pressed = false
				triggered.emit()
			else:
